...
% This function reads out and combines the following data files:
% 1. data .txt file created by LV with belt data
% 2. timestamps .txt file created by LV with belt, scanner (and LFP)
% timestamps
% 3. timestamps .txt file created by NIS Elements to ckeck for consistency
% between both time frames
% 4. Ca-imaging file after NNMF analysis and manual sorting
% Input:
% Pathname - Path were data is located. If all files are in that folder and
% are named consistently the script will go through automatically
% Filename - Name of LV data file
% output: 
...
if nargin < 1 || ~exist([pathname filename '.txt'],'file')
    [filename,pathname] = uigetfile('*.txt','Choose belt data file',pathname);
...
%% load Ca imging data
disp([pathname filename 'Ca.mat'])
if ~exist([pathname filename 'Ca.mat'],'file')
    caim = convertComp(filename);
else
    caim = load([pathname filename 'Ca.mat']);
end    
